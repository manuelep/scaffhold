...
from setupdb.adapters.postgresql import pgsetup, pgteardown
from setupdb.adapters.sqlite import liteteardown
...
class ConnCollector(object):
    ...
    def __init__(self, path, cache=None, time_expire=86400):
        ...
        """
        super(ConnCollector, self).__init__()
        self.path = path
        self.connections = {}
        self.cache = cache
    ...
    def collect(self, _uri, *extensions):
        ...
        try:
            ...
            if uri in self.connections[shp]['extensions']:
                ...
                self.connections[shp]['extensions'][uri] = set(extensions)
