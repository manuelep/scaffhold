...
class ViewInitiator(object):
    """docstring for ViewInitiator."""

    def __init__(self, db=None):
        super(ViewInitiator, self).__init__()
        self.db = db

    def __replace_view(self, name, query, *fields, **options):
        """ """
        sql = """CREATE OR REPLACE VIEW {name} AS {query}"""
        self.db.executesql(sql.format(
            name = name,
            query = query
        ))

    def run(self, name, query, *fields, **options):
        if self.db is None:
            self.db = query._db
        if isinstance(query, str):
            self.__replace_view(name, query)
        else:
            self.__replace_view(
                name,
